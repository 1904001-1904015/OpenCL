...
#define COMPUTE_KERNEL_FILENAME ("./kernal_code.cl") // Path to the kernel file
using namespace std;

int width = 512, height = 512;       // Image resolution, change to your own
...
int gpu = 1;                        // GPU flag, set to 0 for OpenCL computing at your CPU
double avgTime = 0;                 // Statistics info: average time for rendering one image
int counts = 0;                     // Total count of made computations
...
int initMyFilterCl()
{
    ...
    commands = clCreateCommandQueue(context, device_id, 0, &err);  // OpenCL 1.2-compatible command queue
    if (!commands)
    {
    ...
    err = clBuildProgram(program, 0, NULL, "-cl-std=CL1.2", NULL, NULL);  // Ensure CL1.2 is targeted
    if (err != CL_SUCCESS)
    {
        size_t len;
...
void show(const std::string &name, const cv::Mat &mat)
{
    if (!mat.empty())
    {
        cv::namedWindow(name, cv::WINDOW_AUTOSIZE); // Updated to use OpenCV 4.x constant
        cv::imshow(name, mat);
    }
    else
    {
        std::cout << "Fail to load image：Input image is empty!" << std::endl;
    }
}
...
int main()
{
    ...
    Mat rgbimage = imread("./cat.jpg");
    if (!rgbimage.data) // Check if the image is loaded successfully
    {
        cout << "Fail to load image!" << endl;
        return 0;
    }
    Mat grayimage;
    cvtColor(rgbimage, grayimage, cv::COLOR_BGR2GRAY); // Updated OpenCV constant
    initMyFilterCl();
    float filter[3][3] = {
        {-1, 0, -1},
        {0, 4, 0},
    ...
    double duration1;
    std::vector<uchar> outputData(pixelCount, 0); // Replaced VLA with std::vector
    uchar *data;
    start1 = std::clock();
    data = grayimage.data;
    computeMyFilterCl(data, outputData.data());
    duration1 = (std::clock() - start1) / (double)CLOCKS_PER_SEC;
    avgTime += duration1;
    counts++;
    std::cout << "\nOpenCL: time: " << duration1 << " avgTime: " << avgTime / counts << '\n';
    write(fout1, outputData.data(), pixelCount);
    releaseMyFilterCl();
    counts = 0;
    avgTime = 0;
    start2 = std::clock();
    ...
    Mat frameOut3(grayimage.rows, grayimage.cols, CV_8U, outputData.data());
